            ...
            <div class="max-w-3xl mx-auto">
              ...
              <div v-if="feed.type === 'social_media'" class="mb-8">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                  ...
                  <div class="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center space-x-3 mb-4">
                      <div class="w-10 h-10 rounded-full flex items-center justify-center"
                        ...
                        <svg v-if="feed.meta_data?.platform === 'facebook'" class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                        </svg>
                        <svg v-else-if="feed.meta_data?.platform === 'twitter'" class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085a4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                        </svg>
                        <svg v-else-if="feed.meta_data?.platform === 'instagram'" class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <svg v-else-if="feed.meta_data?.platform === 'tiktok'" class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                      ...
                      <div class="flex flex-col">
                        <div class="flex items-center">
                          <span class="text-sm font-medium text-gray-600 dark:text-gray-300 capitalize">{{ feed.meta_data?.platform }}</span>
                          <span v-if="feed.meta_data?.author_name" class="ml-1 text-gray-500 dark:text-gray-400"> • @{{ feed.meta_data.author_name }}</span>
                        </div>
                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ feed.meta_data?.provider_name || feed.site_name }}</span>
                      </div>
                    </div>
                    
                    <!-- Original Title (jika tersedia) - tampilkan maksimal 2 baris -->
                    <div v-if="feed.title || feed.meta_data?.original_title" class="mb-4">
                      <h1 class="text-xl font-semibold text-gray-900 dark:text-white line-clamp-2">
                        {{ feed.title || feed.meta_data?.original_title }}
                      </h1>
                    </div>
                    
                    <!-- Description - tampilkan maksimal 4 baris -->
                    <div v-if="feed.description" class="mb-3 prose prose-sm dark:prose-invert max-w-none">
                      <p class="text-gray-700 dark:text-gray-300 line-clamp-4">{{ feed.description }}</p>
                    </div>
                  </div>
                  
                  <!-- Main Content - memperbesar area konten -->
                  <div class="overflow-hidden">
                    <!-- Embed Content - perbesar height untuk layar yang lebih besar -->
                    ...
                    <!-- Thumbnail jika tidak ada embed atau HTML -->
                  ...
                  <!-- Footer - Tampilkan informasi tertanggal posting -->
