              ...
              <div v-if="feed.type === 'social_media'" class="mb-6 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg">
                ...
                <div class="overflow-hidden">
                  ...
                  <div v-if="feed.meta_data?.embed_url || feed.meta_data?.html" class="social-embed-container">
                    ...
                    <div v-if="feed.meta_data?.platform === 'tiktok'" class="relative w-full h-full">
                      <!-- Thumbnail Preview (Shown First) -->
                      <div v-if="!showTikTokEmbed" class="w-full h-full cursor-pointer" @click="loadTikTokEmbed">
                        <img 
                        ...
                        <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                          <div class="text-white flex flex-col items-center">
                            ...
                            <span class="mt-2">Klik untuk memuat TikTok</span>
                          </div>
                        </div>
                      </div>
                      
                      <!-- Actual TikTok Embed (Loaded on Click) -->
                      <div v-else class="tiktok-embed-container">
                        <div v-html="feed.meta_data?.html"></div>
                      </div>
                    </div>
                    
                    <!-- Instagram specific embed container -->
                    <div v-else-if="feed.meta_data?.platform === 'instagram'" class="relative w-full h-full">
                      <!-- Thumbnail Preview (Shown First) -->
                      <div v-if="!showInstagramEmbed" class="w-full h-full cursor-pointer" @click="loadInstagramEmbed">
                        <img 
                          :src="feed.meta_data?.thumbnail_url" 
                          :alt="feed.title"
                          class="w-full h-full object-cover"
                        ...
                        <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                          <div class="text-white flex flex-col items-center">
                            ...
                            <span class="mt-2">Klik untuk memuat Instagram</span>
                          </div>
                        </div>
                      </div>
                      
                      <!-- Actual Instagram Embed (Loaded on Click) -->
                      <div v-else class="instagram-embed-container">
                        <div v-html="feed.meta_data?.html"></div>
                      </div>
                    </div>
...
const showInstagramEmbed = ref(false)
...
const getIframePermissions = (platform) => {
  switch (platform) {
    case 'facebook':
      return 'allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"';
    case 'twitter':
      return 'allow="web-share"';
    case 'instagram':
      return 'allow="encrypted-media"';
    case 'tiktok':
      return 'allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"';
    case 'youtube':
    case 'youtube_shorts':
      return 'allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"';
    default:
      return '';
  }
}
...
// Fungsi untuk memuat TikTok embed
const loadTikTokEmbed = () => {
  showTikTokEmbed.value = true
  nextTick(() => {
    if (window.TikTok) {
      window.TikTok.reloadEmbeds()
    } else {
...
// Fungsi untuk memuat Instagram embed
const loadInstagramEmbed = () => {
  showInstagramEmbed.value = true
  nextTick(() => {
    if (window.instgrm) {
