            ...
            <form @submit.prevent="submit" class="space-y-6">
              ...
              <div v-if="preview && form.type !== 'image'" class="mt-6">
                ...
                <div class="mt-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                  <div v-if="form.type === 'social_media' && preview.platform" class="space-y-4">
                    ...
                    <div v-if="preview.author_name" class="text-sm text-gray-600 dark:text-gray-300">
                      Oleh: {{ preview.author_name }}
                    </div>
                  </div>
...
const fetchPreview = async (url) => {
  try {
    // Proses URL media sosial terlebih dahulu
    if (form.type === 'social_media') {
      const processedUrl = processSocialMediaUrl(url)
      if (processedUrl) {
...
const processSocialMediaUrl = (url) => {
  try {
    ...
    if (urlObj.hostname.includes('instagram.com')) {
      ...
      const postId = pathParts[2] // Biasanya ID post ada di bagian ketiga path
      if (postId) {
    ...
    if (urlObj.hostname.includes('facebook.com')) {
      // Ubah URL Facebook menjadi format embed
      return url.replace('facebook.com', 'facebook.com/plugins/post')
    }
    
    // Twitter/X
    if (urlObj.hostname.includes('twitter.com') || urlObj.hostname.includes('x.com')) {
      // Ubah URL Twitter menjadi format embed
      return url.replace('twitter.com', 'platform.twitter.com/embed/Tweet.html')
    }
